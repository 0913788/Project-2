    ...
    for row in range(dimension):
        for column in range(dimension):
            ...
            if (row == 0 and column == 1) or (row == 0 and column == 0) or (row == 1 and column == 0):
                node = Tile(Point(column, row), 6, True, "Content\_green_pixel.png", offset)

            elif (row == 0 and column == dimension - 1) or (row == 1 and column == dimension - 1) or (
                    row == 0 and column == dimension - 2):
                node = Tile(Point(column, row), 7, True, "Content\_yellow_pixel.png", offset)

            elif (row == dimension - 2 and column == dimension - 1) or (
                    row == dimension - 1 and column == dimension - 1) or (
                    row == dimension - 1 and column == dimension - 2):
                node = Tile(Point(column, row), 8, True, "Content\_blue_pixel.png", offset)

            elif (row == dimension - 1 and column == 0) or (row == dimension - 2 and column == 0) or (
                    row == dimension - 1 and column == 1):
                node = Tile(Point(column, row), 9, True, "Content\_red_pixel.png", offset)
                ...
                node = Tile(Point(column, row), 10, True, "Content\_fight.png", offset)
            elif (row == dimension - 1 or row == (dimension - 1) / 2) and (
                    column == dimension - 1 or column == (dimension - 1) / 2):
                node = Tile(Point(column, row), 10, True, "Content\_fight.png", offset)
